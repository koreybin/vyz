      ...
      <table class="table" border="1">
        ...
        <tbody>
          <tr v-for="item in tableData" :key="item.rowIdL">
            <td>{{ item.assetsType2 }}</td>
            <td>{{ item.assetsNumber }}</td>
            <td>{{ item.assetsName }}</td>
            <td>{{ item.department }}</td>
            <td>{{ item.monthlyDepreciationRate }}</td>
...
<script>
import { depreciationSchedule } from "@/api/acc/FixedAssets/Card";
export default {
  name: "ScheduleOfDepreciation",
  data() {
    return {
      formulaData: [],
      dialogVisible: false,
  ...
  methods: {
    ...
    getList() {
      depreciationSchedule(this.query.endDate, this.query.startDate).then(
        (res) => {
          res.data.data.forEach((item) => {
            item.monthlyDepreciationRate = item.monthlyDepreciationRate
              ? this.$numeral(item.monthlyDepreciationRate).format("0,0.0000")
              : null;
